...
{
    public class ValidationUpdateSchedule : AbstractValidator<UpdateScheduleRequest>
    {
        private readonly IRepositoryWrapperMariaDB RepositoryWrapperMariaDB;
        public ValidationUpdateSchedule(IRepositoryWrapperMariaDB repositoryWrapperMariaDB)
        {
            ...
            RuleFor(x => x.ScheduleId)
                ...
                .Must((item, scheduleId) => IsExistSchedule(scheduleId)).WithMessage("ScheduleNotFound")
                .WithName("ScheduleId");
            ...
            RuleFor(x => x.ScheduleRepeatValues)
              ...
              .WithName("ScheduleRepeatValues");

            RuleFor(x => x.ScheduleRepeatValueDetails)
              .Cascade(CascadeMode.Stop)
              .NotNull().WithMessage("{PropertyName}IsNotNull")
              .Must((item, scheduleRepeatValueDetails) => CheckSchedule(item.ScheduleId, item.DeviceIds, item.ScheduleDateTimeBeginStr, item.ScheduleDateTimeEndStr, item.ScheduleTimeBeginStr, item.ScheduleTimeEndStr, item.ScheduleRepeat, scheduleRepeatValueDetails)).WithMessage("{PropertyName}Invalid")
        ...
        private bool IsExistSchedule(string id)
        {
            return RepositoryWrapperMariaDB.Schedules.FirstOrDefault(x => x.ScheduleId.Equals(id)) != null;
        }
        ...
        private bool CheckSchedule(string scheduleId, List<string> deviceIds, string dateBegin, string dateEnd, string timeBegin, string timeEnd, ScheduleRepeatEnum scheduleRepeat, List<string> repeatValueDetail)
        {
            if (deviceIds.Count > 0)
            {
                List<string> scheduleIds = RepositoryWrapperMariaDB.ScheduleDevices
                    .FindByCondition(x => deviceIds.Contains(x.DeviceId) && !x.ScheduleId.Equals(scheduleId)).Select(x => x.ScheduleId)
